...
try{
 ...
 const automaticBody={...body,kind:'COST',componentCode:'AUTO_SUPPLIER_ENERGY',monetarySource:'SUPPLIER_ENERGY',scenario:'ACL',timeBand:'ALL',measure:'BRL_MONTH',amount:null,baseRule:'Base automática do fornecedor',startDate:'2028-01-01',endDate:'2028-12-31'};
 const autoDraft=await service.create(automaticBody,'org-a','actor-a');ok(autoDraft.amount_text===null&&autoDraft.monetary_source==='SUPPLIER_ENERGY');
 const autoApproved=await service.approve(autoDraft.id,{revision:1},'org-a','approver');ok(autoApproved.status==='APPROVED');
 ok((await service.events(autoDraft.id,'org-a')).some(e=>e.snapshot.monetary_source==='SUPPLIER_ENERGY'));
 await assert.rejects(()=>db.query("UPDATE calculation_parameters SET monetary_source='SUPPLIER_MINIMUM' WHERE id=$1",[autoDraft.id]),e=>e.code==='P3302');checks++;
 const duplicateAuto=await service.create({...automaticBody,componentCode:'OTHER_DUPLICATE'},'org-a','actor-a');await deny(()=>service.approve(duplicateAuto.id,{revision:1},'org-a','approver'),409);
 for(const patch of [{amount:'1'},{scenario:'ACR'},{kind:'TARIFF'},{direction:'CREDIT'},{measure:'BRL_KWH'},{timeBand:'PEAK'},{monetarySource:'UNKNOWN'}])await deny(()=>service.create({...automaticBody,...patch},'org-a','actor-a'),400);
 await deny(()=>service.create({...automaticBody,consumerUnitId:b},'org-a','actor-a'),404);
