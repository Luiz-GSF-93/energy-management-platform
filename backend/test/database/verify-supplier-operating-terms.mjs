...
try{
 ...
 const operatingMigration=readFileSync(new URL('../../src/database/migrations/20260925_f1_29_supplier_operating_terms.sql',import.meta.url),'utf8');await db.exec(operatingMigration);await db.exec(operatingMigration);ok(true);
 ...
 const schedule=[2026,2027].map(year=>({year,annualVolumeMwh:1200,monthlyPercentages:[10,10,10,10,10,10,10,10,10,10,0,0]}));
 const operating={flexibilityMinPercent:90,flexibilityMaxPercent:110,modulation:'FLEX',submarket:'SE_CO',seasonalityMode:'BOTH',seasonalityRule:'Distribuição anual conforme contrato',seasonalVolumes:schedule};
 const op=await contracts.create({...body,...operating,contractNumber:'Operating'},'org-a');ok(Number(op.flexibility_min_percent)===90&&Number(op.flexibility_max_percent)===110&&op.submarket==='SE_CO'&&op.seasonal_volumes[1].annualVolumeMwh===1200);
 await contracts.update(op.id,'org-a',{guaranteeAmount:12000});ok((await contracts.findOne(op.id,'org-a')).seasonal_volumes.length===2);
 await deny(()=>contracts.update(op.id,'org-b',{modulation:'LOAD_FOLLOWING'}),404);
 const badOperating=[{flexibilityMinPercent:null},{flexibilityMaxPercent:null},{flexibilityMinPercent:-1},{flexibilityMinPercent:120},{modulation:'BAD'},{submarket:'SUDESTE'},{seasonalityMode:'BAD'},{seasonalityRule:''},{seasonalVolumes:null},{seasonalVolumes:[]},{seasonalVolumes:[schedule[0]]},{seasonalVolumes:[schedule[0],schedule[0]]},{seasonalVolumes:[schedule[0],{...schedule[1],year:2028}]},{seasonalVolumes:[{...schedule[0],annualVolumeMwh:-1},schedule[1]]},{seasonalVolumes:[{...schedule[0],unexpected:1},schedule[1]]},{seasonalVolumes:[{...schedule[0],monthlyPercentages:Array(11).fill(0)},schedule[1]]},{seasonalVolumes:[{...schedule[0],monthlyPercentages:Array(13).fill(0)},schedule[1]]},{seasonalVolumes:[{...schedule[0],monthlyPercentages:Array(12).fill(8)},schedule[1]]},{seasonalVolumes:[{...schedule[0],monthlyPercentages:[-1,101,...Array(10).fill(0)]},schedule[1]]},{seasonalityMode:'RULE'}];
 for(const change of badOperating)await deny(()=>contracts.create({...body,...operating,contractNumber:'BAD-OP',...change},'org-a'),400);
 await contracts.update(op.id,'org-a',{status:'ACTIVE'});await deny(()=>contracts.update(op.id,'org-a',{modulation:'LOAD_FOLLOWING'}),409);
 await assert.rejects(()=>db.query("UPDATE energy_contracts SET submarket='N' WHERE id=$1",[op.id]),e=>e.code==='P3292');checks++;
 const partial={consumerUnitId:unit.id,contractNumber:'Partial',contractType:'ENERGY_PURCHASE',contractedVolumeMwh:100,currentPrice:250,startDate:'2026-10-01',endDate:'2026-12-31',seasonalityMode:'MONTHLY',seasonalVolumes:[{year:2026,annualVolumeMwh:100,monthlyPercentages:[0,0,0,0,0,0,0,0,0,30,30,40]}]};
 const part=await contracts.create(partial,'org-a');ok(part.seasonal_volumes[0].monthlyPercentages[11]===40);
 await deny(()=>contracts.create({...partial,contractNumber:'Outside',seasonalVolumes:[{year:2026,annualVolumeMwh:100,monthlyPercentages:[100,...Array(11).fill(0)]}]},'org-a'),400);
 const rule=await contracts.create({...partial,contractNumber:'Rule',seasonalityMode:'RULE',seasonalityRule:'Conforme consumo',seasonalVolumes:[]},'org-a');ok(rule.seasonality_rule==='Conforme consumo');
 await contracts.update(rule.id,'org-a',{seasonalityMode:null,seasonalityRule:null,seasonalVolumes:[]});ok((await contracts.findOne(rule.id,'org-a')).seasonality_mode===null);
 await contracts.update(legacy.id,'org-a',{status:'ACTIVE'});ok((await contracts.findOne(legacy.id,'org-a')).status==='ACTIVE');
