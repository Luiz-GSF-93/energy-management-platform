...
export async function sendMembershipNotification(input: { membershipId: string; email: string; organizationName: string; roleName: string; affiliationType: string }): Promise<MembershipNotificationStatus> {
  ...
  try {
    ...
    const text = ['Você recebeu acesso a uma organização na Expert Energy.', '', 'Organização: '+input.organizationName, 'Função: '+(labels[input.roleName] || input.roleName), 'Vínculo: '+(input.affiliationType==='external'?'Externo / consultor':'Interno à organização'), '', 'Acesse '+login+' com seu e-mail e senha atuais.', 'Se participar de mais de uma organização, selecione a organização desejada após entrar.', 'Se não lembrar sua senha, use Esqueci minha senha na tela de login.', '', 'Este aviso não altera seus acessos nas demais organizações.'].join('\n');
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body:JSON.stringify({from:process.env.MEMBERSHIP_EMAIL_FROM || 'Expert Energy <nao-responda@notificacoes.expertenergy.com.br>',to:[input.email],subject:'Você recebeu acesso a uma organização — Expert Energy',text}),
    });
