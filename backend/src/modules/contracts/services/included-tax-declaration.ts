...
export function includedTaxReferences(p:any,scoped:any[],tariffs:TariffPreview,period:{start:string;end:string}):IncludedTaxReference[]{
 ...
 return items.map((item:any)=>{
  const matches=scoped.filter(r=>r.id===item.parameterId&&r.organization_id===p.organization_id&&r.customer_id===p.customer_id&&r.consumer_unit_id===p.consumer_unit_id&&r.scenario===p.scenario&&r.kind==='TARIFF'),ref=matches[0];
