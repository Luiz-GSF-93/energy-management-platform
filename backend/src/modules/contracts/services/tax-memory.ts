...
export function taxMemory(unit:any,month:string,parameters:any[],tariffs:TariffPreview,operational?:OperationalTaxBases):TaxMemory{
 ...
 const result:TaxMemory={formulaVersion:'tax-memory-1.5',lines:[],declarations:[],pending:[],warnings:[
 ...
 'Tributos já incluídos são conferidos pelas rubricas brutas e seus códigos explícitos; não são extraídos nem acrescentados novamente. Tarifas e origens automáticas do fornecedor/custos exigem memória monetária válida, fonte e revisão. Uma origem automática referencia o componente existente e não cria uma nova despesa.',
 ...
 if(p.tax_basis.interaction!=null&&!['INDEPENDENT','SEQUENTIAL','SHARED_INSIDE'].includes(p.tax_basis.interaction)){reject('Regra de interação tributária não suportada.');continue;}
 if(peers.length&&[p,...peers].some(q=>!['INDEPENDENT','SEQUENTIAL','SHARED_INSIDE'].includes(q.tax_basis?.interaction)||!['INSIDE','OUTSIDE'].includes(q.treatment))){reject('Há outros tributos na mesma rubrica. Declare e aprove a regra de interação em todos os envolvidos, conforme a regra aplicável.');continue;}
 ...
 const lines=baseMemory.lines.filter(l=>l.parameterId===ref.id&&l.revision===ref.revision&&l.scenario===p.scenario);const line=lines[0];
 if((ref.kind!=='TARIFF'&&!(ref.kind==='COST'&&ref.monetary_source&&operational?.lines.some(l=>l.parameterId===ref.id&&l.revision===ref.revision)))||ref.treatment!=='NET'||ref.direction!=='DEBIT'||lines.length!==1||line.treatment!=='NET'||line.startDate>period.start||line.endDate<period.end||baseMemory.pending.some(l=>l.parameterId===ref.id)){error='Uma rubrica incluída não possui memória líquida válida para esta competência. Revise tarifas, medições e tratamento tributário.';break;}
 ...
 return composeTaxes(sharedInsideTaxes(result,taxes),taxes);
}
