...
export type TaxMemory={formulaVersion:'tax-memory-1.0';lines:{id:string;revision:number;label:string;code:string;scenario:string;treatment:string;rate:string;exactBase:string;base:string;amount:string;numerator:string;denominator:string;source:string;references:{id:string;revision:number;label:string;operation:string;exactAmount:string|null}[]}[];declarations:{id:string;label:string;scenario:string;treatment:string;source:string}[];pending:{id:string;label:string;scenario:string;reason:string}[];warnings:string[]};
...
export function taxMemory(unit:any,month:string,parameters:any[],tariffs:TariffPreview):TaxMemory{
 ...
 const result:TaxMemory={formulaVersion:'tax-memory-1.0',lines:[],declarations:[],pending:[],warnings:[
 ...
 'Tributos que compartilham rubricas dependem de regra de interação e ficam bloqueados. Valores já tributados não recebem novo acréscimo.',
 ...
 if(taxes.some(q=>q!==p&&q.scenario===p.scenario&&!['EXEMPT','NOT_APPLICABLE'].includes(q.treatment)&&Array.isArray(q.tax_basis?.items)&&q.tax_basis.items.some((j:any)=>j?.operation==='INCLUDE'&&items.some(i=>i.operation==='INCLUDE'&&i.parameterId===j.parameterId)))){reject('Há outros tributos na mesma rubrica. A interação entre eles precisa de regra explícita antes do cálculo.');continue;}
 ...
 result.lines.push({id:p.id,revision:p.revision,label:p.label,code:p.component_code,scenario:p.scenario,treatment:p.treatment,rate:p.amount_text,exactBase:fixed(base,15),base:rounded(base,SCALE),amount:rounded(numerator,denominator),numerator:numerator.toString(),denominator:denominator.toString(),source:p.source,references});
 }
 return result;
}
