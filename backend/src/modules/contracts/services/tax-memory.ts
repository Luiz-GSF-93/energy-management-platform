...
export function taxMemory(unit:any,month:string,parameters:any[],tariffs:TariffPreview):TaxMemory{
 ...
 const result:TaxMemory={formulaVersion:'tax-memory-1.2',lines:[],declarations:[],pending:[],warnings:[
 ...
 'Rubricas compartilhadas exigem regra explícita. No cálculo sequencial, a base soma somente os tributos selecionados, com valores exatos e revisões aprovadas. Ciclos e denominadores conjuntos não são presumidos. Valores já tributados não recebem novo acréscimo.',
 ...
 if(p.tax_basis.interaction!=null&&!['INDEPENDENT','SEQUENTIAL'].includes(p.tax_basis.interaction)){reject('Regra de interação tributária não suportada.');continue;}
 if(peers.length&&[p,...peers].some(q=>!['INDEPENDENT','SEQUENTIAL'].includes(q.tax_basis?.interaction)||!['INSIDE','OUTSIDE'].includes(q.treatment))){reject('Há outros tributos na mesma rubrica. Declare e aprove a regra independente ou sequencial em todos os envolvidos, conforme a regra aplicável. Composições simultâneas ainda não estão disponíveis.');continue;}
 ...
 return composeTaxes(result,taxes);
}
