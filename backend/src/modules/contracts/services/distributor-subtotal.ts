...
export function distributorSubtotal(unit:any,month:string,parameters:any[],tariffs:TariffPreview,taxes:TaxMemory):DistributorSubtotal{
 return reviewedComponentSubtotal(unit,month,parameters,tariffs,taxes);
}
/** Shared validation for approved tariff/tax components and, optionally, operational sources. */
export function reviewedComponentSubtotal(unit:any,month:string,parameters:any[],tariffs:TariffPreview,taxes:TaxMemory,operational?:OperationalTaxBases):DistributorSubtotal{
 ...
 for(const scenario of ['ACR','ACL'] as const){
  ...
  const own=scoped.filter(p=>p.scenario===scenario),approved=own.filter(p=>p.status==='APPROVED'),tp=approved.filter(p=>p.kind==='TARIFF'||p.kind==='COST'),xp=approved.filter(p=>p.kind==='TAX');
  const lines=baseMemory.lines.filter(l=>l.scenario===scenario),taxLines=taxes.lines.filter(l=>l.scenario===scenario),declarations=taxes.declarations.filter(l=>l.scenario===scenario);
  ...
  for(const p of xp){const ls=taxLines.filter(l=>l.id===p.id&&l.revision===p.revision),ds=declarations.filter(l=>l.id===p.id);
   ...
   if(l.references.some(r=>!tp.some(t=>t.id===r.id&&t.revision===r.revision))){block('Tributo '+p.label+(operational?': a base contém referências fora das rubricas consolidadas. Revise as fontes.':': a base contém rubricas fora da distribuidora. Consulte a composição operacional para conferir tarifas, fornecedor e custos juntos.'));continue;}
