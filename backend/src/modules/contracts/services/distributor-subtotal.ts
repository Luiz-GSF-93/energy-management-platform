...
export type DistributorEntry={id:string;revision:number;kind:'TARIFF'|'TAX';label:string;amount:string;source:string};
export type DistributorScenario={scenario:'ACR'|'ACL';status:'AVAILABLE'|'BLOCKED';tariffs:string|null;taxes:string|null;subtotal:string|null;entries:DistributorEntry[];blockers:string[]};
export type DistributorSubtotal={formulaVersion:'distributor-subtotal-1.0';rounding:'SUM_ROUNDED_LINES';scenarios:DistributorScenario[];warnings:string[]};
...
export function distributorSubtotal(unit:any,month:string,parameters:any[],tariffs:TariffPreview,taxes:TaxMemory):DistributorSubtotal{
 ...
 const result:DistributorSubtotal={formulaVersion:'distributor-subtotal-1.0',rounding:'SUM_ROUNDED_LINES',scenarios:[],warnings:[
  ...
  'Subtotal = soma das tarifas líquidas + soma dos tributos calculados. Cada rubrica conserva seu arredondamento em centavos; as bases e referências dos tributos não são somadas novamente.',
  'Tarifas com tributos embutidos exigem conciliação específica e bloqueiam este subtotal. Cada tributo deve classificar todas as tarifas como incluídas ou excluídas, ou ter declaração aprovada de isenção/não aplicação.',
 ...
 for(const scenario of ['ACR','ACL'] as const){
  const out:DistributorScenario={scenario,status:'BLOCKED',tariffs:null,taxes:null,subtotal:null,entries:[],blockers:[]};result.scenarios.push(out);
  ...
  for(const p of tp){const candidates=lines.filter(l=>l.parameterId===p.id&&l.revision===p.revision),l=candidates[0];
   ...
   if(p.treatment!=='NET'||l.treatment!=='NET'||l.embeddedTaxCodes.length){block('Tarifa '+p.label+': concilie os tributos embutidos antes de somar.');continue;}
   if(!l.source?.trim()){block('Tarifa '+p.label+': fonte ausente.');continue;}
   try{tariffSum+=cents(l.amount);out.entries.push({id:p.id,revision:p.revision,kind:'TARIFF',label:l.label,amount:l.amount,source:l.source});}catch{block('Tarifa '+p.label+': valor inválido.');}
  }
  for(const p of xp){const ls=taxLines.filter(l=>l.id===p.id&&l.revision===p.revision),ds=declarations.filter(l=>l.id===p.id);
   ...
   if(ds.length){if(!['EXEMPT','NOT_APPLICABLE'].includes(p.treatment)||ds[0].treatment!==p.treatment||!ds[0].source?.trim())block('Tributo '+p.label+': declaração incompatível.');continue;}
   const l=ls[0];
