...
const columns: Record<string, string> = {
  flexibilityMinPercent:'flexibility_min_percent',
  flexibilityMaxPercent:'flexibility_max_percent',
  modulation:'modulation',
  submarket:'submarket',
  seasonalityMode:'seasonality_mode',
  seasonalityRule:'seasonality_rule',
  seasonalVolumes:'seasonal_volumes',
...
export class ContractsService {
  ...
  private check(error: any) {
    if (!error) return;
    if (error.code === 'P3291') throw new BadRequestException('Confira os limites de flexibilidade, o submercado e a distribuição sazonal por ano.');
    if (error.code === 'P3292') throw new ConflictException('As condições do contrato ativado são preservadas.');
