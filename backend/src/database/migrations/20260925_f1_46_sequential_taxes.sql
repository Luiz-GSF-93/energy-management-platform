-- F1.46: explicit sequential tax references. No business rows changed.
BEGIN;
...
BEGIN
 ...
 IF NEW.tax_basis IS NOT NULL THEN
  IF NEW.kind<>'TAX' OR NEW.treatment IN ('EXEMPT','NOT_APPLICABLE') OR jsonb_typeof(NEW.tax_basis)<>'object' OR NEW.tax_basis->'version' IS DISTINCT FROM '1'::jsonb OR NEW.tax_basis-ARRAY['version','items','interaction','taxes']<>'{}'::jsonb OR jsonb_typeof(NEW.tax_basis->'items') IS DISTINCT FROM 'array' THEN RAISE EXCEPTION USING ERRCODE='P3311',MESSAGE='Invalid tax basis';END IF;
  IF NEW.tax_basis ? 'interaction' AND (NEW.tax_basis->>'interaction' NOT IN ('INDEPENDENT','SEQUENTIAL') OR jsonb_typeof(NEW.tax_basis->'interaction') IS DISTINCT FROM 'string' OR NEW.treatment NOT IN ('INSIDE','OUTSIDE')) THEN RAISE EXCEPTION USING ERRCODE='P3311',MESSAGE='Invalid tax interaction';END IF;
  IF NEW.tax_basis ? 'taxes' THEN
   ...
   FOR item IN SELECT value FROM jsonb_array_elements(NEW.tax_basis->'taxes') LOOP
    ...
    IF ref.status<>'APPROVED' OR ref.revision<>(item->>'revision')::integer OR ref.treatment NOT IN ('INSIDE','OUTSIDE') OR coalesce(ref.tax_basis->>'interaction','') NOT IN ('INDEPENDENT','SEQUENTIAL') THEN RAISE EXCEPTION USING ERRCODE='P3311',MESSAGE='Approved explicit tax reference required';END IF;
