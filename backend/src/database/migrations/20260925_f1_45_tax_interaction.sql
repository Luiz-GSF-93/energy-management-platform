-- F1.45: explicit independent tax treatment per unit and validity. No business rows changed.
BEGIN;
...
BEGIN
 ...
 IF NEW.tax_basis IS NOT NULL THEN
  IF NEW.kind<>'TAX' OR NEW.treatment IN ('EXEMPT','NOT_APPLICABLE') OR jsonb_typeof(NEW.tax_basis)<>'object' OR NEW.tax_basis->'version' IS DISTINCT FROM '1'::jsonb OR NEW.tax_basis-ARRAY['version','items','interaction']<>'{}'::jsonb OR jsonb_typeof(NEW.tax_basis->'items') IS DISTINCT FROM 'array' THEN RAISE EXCEPTION USING ERRCODE='P3311',MESSAGE='Invalid tax basis';END IF;
  IF NEW.tax_basis ? 'interaction' AND (NEW.tax_basis->'interaction' IS DISTINCT FROM '"INDEPENDENT"'::jsonb OR NEW.treatment NOT IN ('INSIDE','OUTSIDE')) THEN RAISE EXCEPTION USING ERRCODE='P3311',MESSAGE='Invalid tax interaction';END IF;
