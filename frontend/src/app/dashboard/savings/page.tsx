...
export default function SavingsPage() {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">Economia</h1>
        <p className="text-gray-400">Visualize e simule suas economias</p>
      </div>
        ...
        {[
          ...
          <div key={i} className="relative group">
            ...
            <div className="relative p-4">
              ...
              <p className="text-2xl font-bold text-green-400">{card.value}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-white mb-6">Simule suas economias</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {simulations.map((sim) => (
            <button
              key={sim.id}
            ...
            >
              <h3 className="font-semibold text-white mb-1">{sim.name}</h3>
              <p className="text-gray-400 text-sm mb-3">{sim.description}</p>
              <div className="text-2xl font-bold text-green-400">R$ {sim.annualSavings}</div>
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="relative group">
            ...
            <div className="relative p-6">
              <h3 className="text-lg font-semibold text-white mb-6">Detalhes</h3>
              <div className="space-y-4">
                <div>
                  <p className="text-gray-400 text-sm">Consumo Mensal</p>
                  <p className="text-3xl font-bold text-white">{selectedSim.projectedConsumption} kWh</p>
                </div>
                <div>
                  <p className="text-gray-400 text-sm">Custo Anual</p>
                  <p className="text-3xl font-bold text-white">R$ {selectedSim.annualCost}</p>
                </div>
                <div>
                  <p className="text-gray-400 text-sm">Economia</p>
                  <p className="text-3xl font-bold text-green-400">R$ {selectedSim.annualSavings}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="relative group">
            <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-white/5 rounded-xl backdrop-blur-md border border-white/10"></div>
            <div className="relative p-6">
              <h3 className="text-lg font-semibold text-white mb-6">Comparação</h3>
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between mb-2">
                    <p className="text-gray-400">Cenário Atual</p>
                    <p className="text-white">R$ {simulations[0].annualCost}</p>
                  </div>
                ...
                <div>
                  <div className="flex justify-between mb-2">
                    <p className="text-gray-400">Selecionada</p>
                    <p className="text-white">R$ {selectedSim.annualCost}</p>
                  </div>
                  <div className="h-2 bg-white/10 rounded-full overflow-hidden">
      ...
      <div className="relative group">
        <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-white/5 rounded-xl backdrop-blur-md border border-white/10"></div>
        <div className="relative p-6">
          <h3 className="text-lg font-semibold text-white mb-6">Estratégias Recomendadas</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {strategies.map((s, i) => (
              <div key={i} className="p-4 bg-white/5 border border-white/10 rounded-lg">
                ...
                <p className="text-green-400 font-semibold text-sm">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
