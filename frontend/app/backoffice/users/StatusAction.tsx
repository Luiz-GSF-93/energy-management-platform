...
export default function StatusAction({userId,email,status,disabled,onChanged}:{userId:string;email:string;status:string;disabled:boolean;onChanged:()=>Promise<void>}) {
 ...
 async function change(){if(busy)return;setBusy(true);setError('');
  try{await apiRequest('/api/v1/admin/users/'+encodeURIComponent(userId)+'/status',{method:'PATCH',body:{status:activating?'active':'inactive'}});setConfirming(false);await onChanged();}
 ...
 <p>{activating?'O usuário voltará a acessar esta organização com sua função atual, respeitando o limite de usuários da licença.':'O usuário perderá o acesso a esta organização. Os documentos, o histórico e os vínculos em outras organizações serão preservados.'}</p>
