...
function Setup(){
 ...
 async function save(e:FormEvent<HTMLFormElement>,unit:boolean){e.preventDefault();if(busy)return;const form=e.currentTarget;const f=new FormData(form);const body=Object.fromEntries(Array.from(f.entries()).map(([k,v])=>[k,String(v).trim()]).filter(([,v])=>v!==''));setBusy(true);setError('');setMessage('');
 ...
 <Input label="Razão social" name="company_name" required disabled={busy}/><Input label="Nome fantasia" name="trade_name" disabled={busy}/><Input label="CPF / CNPJ" name="document" required disabled={busy}/><Input label="Contato" name="contact_name" disabled={busy}/><Input label="E-mail" name="contact_email" type="email" disabled={busy}/><Input label="Telefone" name="contact_phone" disabled={busy}/><Button type="submit" disabled={busy}>Cadastrar cliente</Button></form></Card>:null}
