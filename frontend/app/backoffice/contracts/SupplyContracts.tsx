...
type Contract=SupplyTerms & {id:string;customer_id:string;consumer_unit_id:string;contract_number:string;contract_type:string;contracted_volume_mwh:number;current_price:number;start_date:string;end_date:string;status:string;supplier_id?:string;energy_source?:string;adjustment_index?:string;adjustment_frequency?:string;notes?:string};
...
export default function SupplyContracts({customerId,onDirty}:{customerId:string;onDirty:(dirty:boolean)=>void}){
 ...
 const body:Record<string,unknown>={...terms,endDate:end,contractedVolumeMwh:volume,currentPrice:price,notes:text('notes')};
