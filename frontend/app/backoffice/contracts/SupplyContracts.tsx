...
export default function SupplyContracts({customerId,onDirty}:{customerId:string;onDirty:(dirty:boolean)=>void}){
 ...
 setRows(old=>[row,...old.filter(c=>c.id!==row.id)]);setMessage(editing?'Rascunho atualizado.':'Contrato salvo como rascunho. Revise os dados antes de ativar.');setEditing(null);form.reset();onDirty(false);}
 ...
 {!editing&&(!viewCustomers||!viewUnits)?<p>O cadastro requer permissão de consulta de clientes e unidades consumidoras.</p>:<form key={editing?.id||'new'} onSubmit={save} onChange={()=>onDirty(true)} className="organizations-create__form">
 {!editing?<>
 ...
 {c.status==='DRAFT'&&update&&['ENERGY_PURCHASE','ENERGY_SALE'].includes(c.contract_type)?<><Button variant="secondary" disabled={busy} onClick={()=>{setEditing(c);setActivating(null);setError('');setMessage('');window.scrollTo({top:0,behavior:'smooth'});}}>Editar rascunho</Button>
