...
export type ComparisonInput={tariffPreview?:TariffPreviewData;taxMemory?:TaxMemoryData;costLedger?:MonthlyCostLedgerData;catalog:{key:string;label:string;scenario:string;kind:string;timeBand:string;versions:{id:string}[]}[]};
export type ComparisonEntry={state:'VALUE'|'PENDING'|'DECLARED';label:string;amount?:string;detail:string;source?:string;revision?:number;recordId?:string;kind?:'TARIFF'|'TAX'|'COST'};
