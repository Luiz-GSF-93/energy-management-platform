...
const tabs=[['distributor','Distribuidora e unidades'],['supply','Fornecedor Mercado Livre'],['management','Honorários da gestão'],['services','Intermediação e outros'],['parameters','Parâmetros de cálculo'],['preparation','Preparar apuração'],['monthly','Dados mensais']] as const;
function Workspace(){const {hasPermission}=useAuth();const [customers,setCustomers]=useState<Customer[]>([]),[units,setUnits]=useState<Unit[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(''),[revision,setRevision]=useState(0),[customerId,setCustomer]=useState(''),[tab,setTab]=useState<string>('distributor'),[dirty,setDirty]=useState(false),[search,setSearch]=useState('');
 ...
 <label>Cliente em operação<select className="ds-input" value={customerId} onChange={e=>change(()=>setCustomer(e.target.value))}><option value="">Todos os clientes — visão de consulta</option>{customers.filter(c=>c.id===customerId||c.company_name.toLocaleLowerCase().includes(search.toLocaleLowerCase())).map(c=><option key={c.id} value={c.id}>{c.company_name}</option>)}</select></label>
 ...
 <div key={customerId+'-'+tab}>{tab==='distributor'?<Distributor customerId={customerId} units={units} customers={customers} onUnits={setUnits} onDirty={setDirty}/>:tab==='supply'?<SupplyContracts customerId={customerId} onDirty={setDirty}/>:tab==='monthly'?<MonthlyInputs customerId={customerId} units={units} onDirty={setDirty}/>:tab==='preparation'?<CalculationPreparation customerId={customerId} units={units}/>:tab==='parameters'?<CalculationParameters customerId={customerId} units={units} onDirty={setDirty}/>:<CommercialTerms kind={tab as 'management'|'services'} customerId={customerId} units={units} customers={customers} onDirty={setDirty}/>}</div>
 </>:null}</section>;
}
