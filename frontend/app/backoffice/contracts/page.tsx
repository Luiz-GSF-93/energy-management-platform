...
function Workspace(){const {hasPermission}=useAuth();const [customers,setCustomers]=useState<Customer[]>([]),[units,setUnits]=useState<Unit[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(''),[revision,setRevision]=useState(0),[customerId,setCustomer]=useState(''),[tab,setTab]=useState<string>('distributor'),[dirty,setDirty]=useState(false),[search,setSearch]=useState('');
 ...
 <div key={customerId+'-'+tab+'-'+listVersion}>{tab==='distributor'?<Distributor allowNew={false} customerId={customerId} units={units} customers={customers} onUnits={setUnits} onDirty={setDirty}/>:tab==='supply'?<SupplyContracts allowNew={false} customerId={customerId} onDirty={setDirty}/>:tab==='costs'?<MonthlyCosts customerId={customerId} units={units} onDirty={setDirty}/>:tab==='monthly'?<MonthlyInputs customerId={customerId} units={units} onDirty={setDirty}/>:tab==='preparation'?<CalculationPreparation customerId={customerId} units={units}/>:tab==='parameters'?<CalculationParameters customerId={customerId} units={units} onDirty={setDirty}/>:<CommercialTerms allowNew={false} kind={tab as 'management'|'services'} customerId={customerId} units={units} customers={customers} onDirty={setDirty}/>}</div></>:null}
 </>:null}</section>;
}
