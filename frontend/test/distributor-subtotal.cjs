...
(async()=>{try{
 ...
 await render({...data,scenarios:[{...ready,tariffs:'123.46',taxes:'0.00',subtotal:'123.46',embeddedTaxCodes:['ICMS','PIS'],entries:[{id:'gross',revision:3,kind:'TARIFF',label:'Energia bruta',amount:'123.46',source:'Fonte bruta',treatment:'GROSS',embeddedTaxCodes:['ICMS','PIS']}]}]});ok(text().includes('Já inclusos nas tarifas: ICMS, PIS'),'embedded taxes disclosed');ok(text().includes('Sem novo acréscimo; valor embutido não desmembrado'),'does not assert zero tax');ok(text().includes('Tarifas aprovadas')&&text().includes('Tributos a acrescentar')&&!text().includes('Tarifas líquidas'),'gross values not mislabeled net');ok(text().includes('Tarifa com tributos inclusos: ICMS, PIS')&&text().includes('Revisão 3'),'per-source embedded tax evidence');
